#!/usr/bin/env python3
"""Experiment 44a: spatially blocked blind reconstruction in one Apis patch.
...
def main() -> None:
    ...
    parser.add_argument("--voxel-size-micrometres", type=float, default=1.6)
    ...
    outer_all = detect_outer_centres(data[:, :, 0], roi)
    ...
    for fold in range(5):
        ...
        for local_index, sample_index in enumerate(test_indices):
            ...
            predicted_centre = predicted_affine[local_index]
    ...
    summary: dict[str, object] = {
        "status": "exploratory pilot; not independent validation",
        ...
        "voxel_calibration_note": (
            "Uses the 1.6 micrometre bee-eye acquisition voxel size reported in "
            "Tichit et al. (2022); the supplied NIfTI label header is unitless."
        ),
        "centre_mapping": {},
        "reconstruction": {},
    }
    ...
    figure.suptitle("Experiment 44a: spatial-block held-out cone reconstruction", fontsize=12)
    figure.tight_layout()
